...
const listarActividadesProyecto = async function (req, res) {
    if (req.user) {
        try {
            ...
            var actividadesProyecto = await Model.Actividad_proyecto.find(filtroConsulta).sort({ createdAt: -1 });
...
const listarTiposActividadesProyecto = async function (req, res) {
    if (req.user) {
        try {
            ...
            var tiposActividadesProyecto = await Model.Tipo_actividad_proyecto.find(filtroConsulta).sort({ createdAt: -1 });
