...
const actualizarActividadProyecto = async function (req, res) {
    if (req.user) {
        var id = req.params['id'];
        try {
            let actividadActualizada = await Model.Actividad_proyecto.findByIdAndUpdate(id, req.body, { new: true });
...
const actualizarTipoActividadProyecto = async function (req, res) {
    if (req.user) {
        var id = req.params['id'];
        try {
            let tipoActualizado = await Model.Tipo_actividad_proyecto.findByIdAndUpdate(id, req.body, { new: true });
